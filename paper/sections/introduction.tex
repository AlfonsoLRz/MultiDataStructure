...
\begin{figure*}[t]
  ...
  \begin{tikzpicture}[x=1mm, y=1mm]
    % Explicit coordinates: the loop arrow and the two artifact boxes need lanes
    % reserved for them, which relative placement does not give.
    \node[mdsbox, minimum width=22mm] (cloud)    at (0,  6) {point cloud};
    \node[mdsbox, minimum width=22mm] (trace)    at (0, -6) {application query trace};
    \node[mdsbox, minimum width=20mm] (features) at (30, 0) {feature\\extraction};
    \node[mdsbox, minimum width=26mm] (pool)     at (62, 0)
      {candidate pool\\{\tiny single $\cdot$ nested $\cdot$ conditional}};
    \node[mdsmeasured, minimum width=42mm] (measure) at (62, -18)
      {build over the cloud, replay the trace\\{\tiny CPU / CUDA $\cdot$ latency and traversal counters}};

    \node[mdsartifact, minimum width=24mm] (schema) at (108,  6) {selected schema\\{\tiny replayable JSON}};
    \node[mdsartifact, minimum width=24mm] (rows)   at (108, -6) {measurement rows\\{\tiny with provenance}};

    \draw[mdsflow] (cloud.east) -| (features.north);
    \draw[mdsflow] (trace.east) -| (features.south);
    \draw[mdsflow] (features) -- (pool);
    \draw[mdsflow] (pool) -- (measure);
    % The loop returns down the left of the search box, clear of both nodes.
    % Left of both boxes (pool spans 49..75, measure 41..83) but inside the dashed
    % region, so the return path crosses nothing.
    \draw[mdsloop] (measure.west) -- (37,-18) -- (37,0)
      node[mdslabel, pos=0.5, left=0.3mm, text=mdsaccent, align=center] {propose\\next}
      -- (pool.west);
    \draw[mdsflow] (pool.east)    -- (86,0) |- (schema.west);
    \draw[mdsflow] (measure.east) -- (86,-18) |- (rows.west);

    \begin{scope}[on background layer]
      \node[draw=mdsaccent, dashed, rounded corners=2pt, inner sep=0pt,
            fit=(pool)(measure), inner xsep=8mm, inner ysep=3mm] (loop) {};
    \end{scope}
    \node[mdslabel, below=0.5mm of loop.south, text=mdsaccent] {measured search loop};
  \end{tikzpicture}
  \caption{Overview of the workload-aware schema synthesis workflow. The optimizer is a measured loop rather than an analytical predictor: every candidate is built over the target cloud, replayed against the recorded trace, and scored from its own latency and traversal counters. The outputs that leave the system are a replayable schema and a provenance-carrying measurement table.}
  \label{fig:pipeline}
\end{figure*}
