...
`core/Config.*` parses JSON schema files into `SchemaConfig`. The parser supports `QuadTree`, `Octree`, `KarrasOctree`, `KDTree`, `BIH`, `BVH`, `LBVH`, `RegularGrid`, `HGrid`, and `Mixed` schema names. GPU-specific aliases keep their primitive identity for CUDA replay while carrying an explicit CPU fallback family. `QuadTree` levels can set `axisPolicy` to `xy`, `xz`, `yz`, `ignore_shortest`, `ignore_x`, `ignore_y`, or `ignore_z`; point-cloud schemas default to `xy`. `KDTree`/`BIH` levels can set `median_longest_axis`, `center_longest_axis`, or `round_robin`; CUDA comparability is tagged per row with `gpu_support_status`.
...
- `PointCloud`: vector-backed point set with private stats, private `.xyz`/`.csv`/ASCII `.ply`/uncompressed `.las` loaders, and a sibling `.mdspc` binary cache. LAS coordinates are converted to local float positions relative to a double-precision coordinate-frame origin so geospatial offsets do not erase centimeter-scale detail.
...
`experiments/SchemaSearch.*` runs measured schema sweeps. It defaults to CUDA/Mixed evaluation on device `0`, the lightweight 3D volume workload, 64 prepared workload queries, 256 generated candidates, and top-32 JSON-ranker pruning, then falls back to CPU with a warning if CUDA is unavailable. It can evaluate configured schema JSON files or generate nested schema candidates from bounded intervals over the currently implemented point node types. CPU discovery uses a query-minimal primitive profile (`QuadTree`, `Octree`, `KDTree`, `BVH`) so CPU-equivalent GPU aliases do not dilute search; CUDA/full profiles keep `KarrasOctree`, `BIH`, `LBVH`, `RegularGrid`, and `HGrid`. Generated schemas are written as replayable JSON files, then optionally pruned by a selector model before measurement. The `--auto-conditions` path estimates a deterministic shallow point-cloud sketch, builds condition-threshold domains, stages proxy/full/confirmation measurements, saves the winning numeric schema JSON, and appends condition summaries plus score/objective provenance to the CSV rows. The `--deep-nested-search` path is CPU-first for discovery: it injects pure CPU-distinct single-DS baselines, runs a nested-opportunity diagnostic, generates template-guided nested families, stages proxy/shortlist/confirmation/robustness measurements, records active structure usage by nodes and leaf points, and optionally reports CUDA/Mixed confirmation. Each measured run logs one raw row per `(dataset, workload, schema)`, computes the scalar score, tags GPU support status, and writes best-schema rows per `(dataset, workload)`.
