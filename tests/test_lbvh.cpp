...
#include "../MultiDataStructure/workloads/points/LBVH.h"
...
namespace BaselineTests
{
	...
	namespace
	{
		SchemaConfig makeLBVHSchema()
		{
			...
			SchemaConfig schema;
			schema._name = "lbvh_test";
			schema._levels.push_back(level);
			schema._buildPolicy._maxDepth = 12;
	...
	void runLBVHTests()
	{
		...
		if (!PointGpu::LBVH::isAvailable(&cudaError))
		{
			std::cout << "LBVH tests skipped: " << cudaError << '\n';
			return;
		}

		const PointCloud cloud = SyntheticPointClouds::generateSparseDenseMixture(64, 128, 29);
		PointGpu::LBVH index;
		const PointGpu::BuildResult build = index.build(cloud, makeLBVHSchema());
		expect(build._metrics._indexedPoints == cloud.size(), "LBVH indexes every point");
		expect(build._metrics._numLeaves > 0, "LBVH creates leaves");
		expect(build._metrics._numNodes >= build._metrics._numLeaves, "LBVH creates a valid node array");
		const PointGpu::BuildResult rebuild = index.build(cloud, makeLBVHSchema());
		expect(rebuild._uploadTimeMs == 0.0, "LBVH reuses uploaded points on repeated builds");
		...
		expect(result._samples.size() == queries.size(), "LBVH returns one sample per query");
		expect(result._samples[0]._returnedPoints == bruteForceRangeCount(cloud, range._bounds), "LBVH range count matches brute force");
		expect(result._samples[1]._returnedPoints == bruteForceRangeCount(cloud, countRange._bounds), "LBVH count-range matches brute force");
		expect(result._samples[2]._returnedPoints == bruteForceRadiusCount(cloud, radius.center, radius._radius), "LBVH radius count matches brute force");
		expect(result._samples[3]._returnedPoints == std::min(knn._k, cloud.size()), "LBVH KNN returns requested neighbor count");
		expect(result._samples[3]._testedPoints == cloud.size(), "LBVH KNN scans the GPU point buffer");
		expect(result._knnQueries == 1, "LBVH counts KNN queries");
		expect(result._metrics._totalQueries == queries.size(), "LBVH summarizes query samples");
	}
}
